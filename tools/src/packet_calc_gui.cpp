...
void PacketCalcGUI::setupUI()
{
    ...
    connect(ui->packetDataTextEdit, &QTextEdit::textChanged, this, &PacketCalcGUI::onPacketDataChanged);
    connect(ui->clientIdSpinBox, QOverload<int>::of(&QSpinBox::valueChanged), this, &PacketCalcGUI::onPacketDataChanged);
    connect(ui->modeSpinBox, QOverload<int>::of(&QSpinBox::valueChanged), this, &PacketCalcGUI::onPacketDataChanged);
}
...
void PacketCalcGUI::connectToDevice()
{
    ...
    if (interface->init_serial(port.toStdString(), baudrate)) {
        interface->set_data_callback([this](const Packet& recv_pkt) {
...
uint8_t PacketCalcGUI::calculateCRC(uint8_t client_id, uint8_t command, const std::vector<uint8_t>& data)
{
...
void PacketCalcGUI::sendCustomPacket()
{
    ...
    if (success && response.is_valid) {
        ui->responseModeLabel->setText(QString("Mode: 0x%1").arg(response.mode, 2, 16, QChar('0')).toUpper());
        ui->responseCrcLabel->setText(QString("CRC: 0x%1").arg(response.crc, 2, 16, QChar('0')).toUpper());
        ...
        logMessage(QString("Custom packet sent successfully - Command: 0x%1, Response length: %2")
                   .arg(packet.command, 2, 16, QChar('0'))
